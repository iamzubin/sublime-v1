...
contract SavingsAccount is ISavingsAccount, Initializable, OwnableUpgradeable, ReentrancyGuard {
    ...
    function depositTo(
        ...
        address strategy,
        address to
    ) external override nonReentrant returns (uint256 sharesReceived) {
    ...
    function switchStrategy(
        ...
        require(asset != address(0), 'asset cannot be address(0)');
        require(currentStrategy != address(0), 'Savings Accountt::currentStrategy cannot be address(0)');
        require(newStrategy != address(0), 'Savings Accountt::currentStrategy cannot be address(0)');
    ...
    function withdrawAll(address _asset) external override nonReentrant returns (uint256 tokenReceived) {
        ...
        _transfer(_asset, msg.sender, tokenReceived);
    ...
    function approve(
        ...
        require(token != address(0), 'asset cannot be address(0)');
