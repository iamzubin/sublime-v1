...
describe('CreditLine, Borrow Token: USDC, CollateralToken: ETH', async () => {
    ...
    it('Sample', async function () {
        ...
        let poolAddress = await calculateNewPoolAddress(env, USDC, ETH, iyield, salt, false, {
        ...
        let pool = await createNewPool(env, USDC, ETH, iyield, salt, false, {
            _poolSize: BigNumber.from(100).mul(BigNumber.from(10).pow(6)), // max possible borrow tokens in pool
            _borrowRate: BigNumber.from(5).mul(BigNumber.from(10).pow(28)), // 100 * 10^28 in contract means 100% to outside
            _collateralAmount: BigNumber.from(1).mul(BigNumber.from(10).pow(18)), // 1 eth
            _collateralRatio: BigNumber.from(250).mul(BigNumber.from(10).pow(28)), //250 * 10**28
...
describe(`Credit Lines ${Contracts.USDC}/${zeroAddress}: Calculate Borrowable Amount`, async () => {
    ...
    it('If no collateral is deposited, then borrowable amount should be 0, autoliquidation = false', async () => {
        ...
        let collateralDecimals = BigNumber.from(18);
...
describe.skip(`Credit Lines ${Contracts.USDC}/${zeroAddress}: Liquidate Credit Lines`, async () => {
    let env: Environment;
    let creditLine: CreditLine;
    let admin: SignerWithAddress;
    let lender: SignerWithAddress;
