...
describe(`Credit Lines ${zeroAddress}/${Contracts.WBTC}: Liquidate Credit Lines`, async () => {
    ...
    beforeEach(async () => {
        ...
        await env.savingsAccount.connect(lender).approve(borrowLimit, BorrowAsset.address, creditLine.address);
        let borrowableAmount = await creditLine.connect(borrower).callStatic.calculateBorrowableAmount(creditLineNumber);
        // console.log({ borrowableAmount: (borrowableAmount.mul(95).div(100)).toString() });
        await creditLine.connect(borrower).borrow(creditLineNumber, borrowableAmount.mul(95).div(100)); // 95% of borrow limit
    ...
    it('Test Liquidation: should be liquidated if collateral ratio goes below', async () => {
        ...
        let randomBalanceAfter = await ethers.provider.getBalance(random.address);
    ...
    it('Test Liquidation: Random address should be liquidated if collateral ratio goes below', async () => {
        let random = env.entities.extraLenders[10];
        // move blocks to be able to liquidate due to change in collateral ratio
        await incrementChain(network, 2000, 150000000);
    ...
    it('Test Liquidation: Lender should be liquidated if collateral ratio goes below', async () => {
        // move blocks to be able to liquidate due to change in collateral ratio
        await incrementChain(network, 2000, 150000000);
