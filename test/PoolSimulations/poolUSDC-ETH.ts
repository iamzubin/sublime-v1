...
describe('Pool, Strategy: Compound, Borrow Token: USDT, CollateralToken: ETH', async () => {
    ...
    before(async () => {
        env = await createEnvironment(
            ...
            [
                ...
                { asset: WETH9, liquidityToken: Contracts.cETH },
            ] as CompoundPair[],
            [] as YearnPair[],
            [
                { tokenAddress: WETH9, feedAggregator: ChainLinkAggregators['ETH/USD'] },
    ...
    it('Sample', async function () {
        ...
        let ETH: ERC20 = await deployHelper.mock.getMockERC20(WETH9); // this is made into type only for matching the signature
        ...
        await env.mockTokenContracts[1].contract.connect(env.impersonatedAccounts[0]).transfer(admin.address, '1000000000000000000');
        await env.mockTokenContracts[1].contract.connect(admin).transfer(borrower.address, '1000000000000000000');
        await env.mockTokenContracts[1].contract.connect(borrower).approve(poolAddress, '1000000000000000000');
