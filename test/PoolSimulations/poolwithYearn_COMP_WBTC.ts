...
describe('Pool using Yearn Strategy with COMP (Borrow Token) and WBTC (Collateral Token)', async () => {
    let env: Environment;
    before(async () => {
        env = await createEnvironment(
            ...
            [] as CompoundPair[],
            [
                { asset: COMP_CURVE_POOL, liquidityToken: COMP_Yearn_Protocol_Address },
                { asset: Contracts.WBTC, liquidityToken: WBTC_Yearn_Protocol_Address },
            ] as YearnPair[],
            [
                { tokenAddress: COMP_CURVE_POOL, feedAggregator: ChainLinkAggregators['COMP/USD'] },
    ...
    it('Sample', async function () {
        ...
        let COMP: ERC20 = await deployHelper.mock.getMockERC20(COMP_CURVE_POOL);
        ...
        let poolAddress = await calculateNewPoolAddress(env, COMP, WBTC, iyield, salt, false, {
        ...
        let pool = await createNewPool(env, COMP, WBTC, iyield, salt, false, {
            _poolSize: BigNumber.from(100).mul(BigNumber.from(10).pow(18)),
            _minborrowAmount: BigNumber.from(10).mul(BigNumber.from(10).pow(18)),
            _borrowRate: BigNumber.from(1).mul(BigNumber.from(10).pow(28)),
            _collateralAmount: BigNumber.from(1).mul(BigNumber.from(10).pow(8)),
