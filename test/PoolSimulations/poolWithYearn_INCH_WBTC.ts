...
describe('Pool using Yearn Strategy with INCH (Borrow Token) and WBTC (Collateral Token)', async () => {
    let env: Environment;
    before(async () => {
        env = await createEnvironment(
            ...
            [] as CompoundPair[],
            [
                { asset: INCH_Token_Address, liquidityToken: INCH_Yearn_Protocol_Address },
                { asset: Contracts.WBTC, liquidityToken: WBTC_Yearn_Protocol_Address },
            ] as YearnPair[],
            [
                { tokenAddress: INCH_Token_Address, feedAggregator: ChainLinkAggregators['INCH/USD'] },
    ...
    it('Sample', async function () {
        ...
        let INCH: ERC20 = await deployHelper.mock.getMockERC20(INCH_Token_Address);
        ...
        let poolAddress = await calculateNewPoolAddress(env, INCH, WBTC, iyield, salt, false, {
        ...
        let pool = await createNewPool(env, INCH, WBTC, iyield, salt, false, {
            _poolSize: BigNumber.from(100).mul(BigNumber.from(10).pow(18)),
            _minborrowAmount: BigNumber.from(10).mul(BigNumber.from(10).pow(18)),
            _borrowRate: BigNumber.from(1).mul(BigNumber.from(10).pow(28)),
            _collateralAmount: BigNumber.from(1).mul(BigNumber.from(10).pow(8)),
