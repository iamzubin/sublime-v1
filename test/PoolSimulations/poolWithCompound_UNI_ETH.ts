...
describe('Pool using Compound Strategy with UNI as borrow token and WBTC as collateral', async () => {
    ...
    before(async () => {
        ...
        await priceOracle.connect(admin).setChainlinkFeedAddress(zeroAddress, ChainLinkAggregators['ETH/USD']);
    ...
    async function createPool() {
        ...
        let generatedPoolAddress: Address = await getPoolAddress(
            ...
            poolLogic.address,
            false,
            { _collateralAmount: createPoolParams._collateralAmountForETH }
        );
        ...
        let {
            ...
            _collateralAmountForETH: _collateralAmount,
        } = createPoolParams;
    ...
    describe('Check ratios', async () => {
        ...
        it('Check Ratio after borrowing borrow total 10 UNI with 1 ETH Collateral', async () => {
            let deployHelper = new DeployHelper(borrower);
            let token: PoolToken = await deployHelper.pool.getPoolToken(UNITokenContract.address);
            let decimals = await token.decimals();
            let expDecimals = BigNumber.from(10).pow(decimals);
            ...
            let token1: PoolToken = await deployHelper.pool.getPoolToken(zeroAddress);
